...
func (s *Store) List(ctx context.Context, prefix string) ([]string, error) {
	keyPrefix := "/ctsnarf/secrets/store/"
	if prefix != "" {
		keyPrefix += prefix
	}
	...
	for _, kv := range resp.Kvs {
		...
		keys = append(keys, strings.TrimPrefix(k, "/ctsnarf/secrets/store/"))
	}
	return keys, nil
}
...
func (n *Store) Set(ctx context.Context, key string, value []byte) error {
	...
	_, err := n.etcd.Put(ctx, "/ctsnarf/secrets/store/"+key, b64)
	return err
}
...
func (n *Store) Get(ctx context.Context, key string) ([]byte, error) {
	resp, err := n.etcd.Get(ctx, "/ctsnarf/secrets/store/"+key)
...
func (s *Store) Delete(ctx context.Context, key string) error {
	_, err := s.etcd.Delete(ctx, "/ctsnarf/secrets/store/"+key)
	return err
}
