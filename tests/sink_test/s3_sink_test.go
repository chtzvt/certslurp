...
func TestS3Sink_CompressedPayloads(t *testing.T) {
	store := setupTestStore(t)
	ctx := context.Background()
	require.NoError(t, store.Set(ctx, "TEST_AWS_ACCESS_KEY_ID", []byte("fake-access")))
	require.NoError(t, store.Set(ctx, "TEST_AWS_SECRET_ACCESS_KEY", []byte("fake-secret")))
	...
	for _, cc := range []compressionCase{
		{"none", "none"},
		{"gzip", "gzip"},
		{"bzip2", "bzip2"},
		{"zstd", "zstd"},
		...
		t.Run(cc.name, func(t *testing.T) {
			...
			payload := []byte("payload for s3 sink")
			var buf testutil.WriteCloserBuffer
			if cc.compression == "none" {
				buf.Write(payload)
			} else {
				compWriter, err := compression.NewWriter(&buf, cc.compression)
				require.NoError(t, err)
			...
			require.True(t, mock.called)
			require.Equal(t, "prefix/testfile.txt", mock.lastKey)
			require.Equal(t, buf.Bytes(), mock.lastBody)
		})
	}
}
